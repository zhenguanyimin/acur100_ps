...
static void adc_data_test_fill_data(uint16_t fixVal)
{
	uint16_t crc = 0;
	uint32_t i = 0;
...
static BaseType_t adc_data_test_cmd_handler(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	...
	if (len3 > 0)
	{
		fixVal = (uint16_t)strtoul(param3, NULL, 0);
	}
